//! Raydium Launchpad (LaunchLab) decoder.
//!
//! Source: `docs/dex-reference.md` §4 (official IDL
//! `github.com/raydium-io/raydium-idl/raydium_launchpad/raydium_launchpad.json`,
//! `raydium_launchpad` v0.2.0) and §0 (cross-cutting decode strategy). Per §0,
//! the DEX is identified by program ID upstream (the router), and amounts come
//! from `meta` balance deltas — never from instruction args, which are only
//! slippage limits (`amount_in`/`minimum_amount_out`/`share_fee_rate`, etc.).
...
/// Raydium Launchpad / LaunchLab token-launch (bonding-curve) program.
/// Source: docs/dex-reference.md §4.
///
/// NOTE: This is the Launchpad/LaunchLab program. It must NOT be confused with
/// Raydium AMM v4, CLMM, or CPMM — those are separate programs with separate IDs.
pub const PROGRAM_ID: &str = "LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj";

// Discriminators = sha256("global:<name>")[..8]; source docs/dex-reference.md §4.
// Both buy variants map to Buy; both sell variants map to Sell.
...
pub fn decode(
    ...
    // 1. Defensive: confirm this instruction targets Raydium Launchpad.
    if instruction.program_id() != Some(PROGRAM_ID) {
        return None;
    }

    // 2. Discriminator → action. Four variants: any buy_* → Buy, any sell_* →
    //    Sell. Anything else (initialize, migrate, etc.) is skipped.
    let disc = common::discriminator(instruction.data.as_deref()?)?;
    let action = match disc {
        DISC_BUY_EXACT_IN | DISC_BUY_EXACT_OUT => TradeAction::Buy,
        DISC_SELL_EXACT_IN | DISC_SELL_EXACT_OUT => TradeAction::Sell,
        _ => return None,
    };
    ...
    Some(DecodedTradeEvent {
        ...
        dex: DexProtocol::RaydiumLaunchpad,
        action,
        mint,
        sol_amount,
        token_amount,
...
mod tests {
    ...
    fn instruction(disc: &[u8; 8]) -> UiInstruction {
        // amount_in + minimum_amount_out + share_fee_rate-ish; ignored by decoder.
        let some_args = [0u8, 0, 0, 0, 0, 0, 0, 0, 1];
    ...
    #[test]
    fn decodes_buy_exact_in_with_amounts_from_balance_deltas() {
        ...
        let event =
            decode(&result, &instruction(&DISC_BUY_EXACT_IN), &whales).expect("buy decodes");
    ...
    #[test]
    fn decodes_sell_exact_in_with_amounts_from_balance_deltas() {
        ...
        let event =
            decode(&result, &instruction(&DISC_SELL_EXACT_IN), &whales).expect("sell decodes");
    ...
    #[test]
    fn decodes_buy_exact_out_variant_maps_to_buy() {
        // Proves the *_exact_out variants map correctly (here buy_exact_out → Buy).
        let result = build_result(
            vec![10_000_000_000, 0],
            vec![6_500_000_000, 0],
            vec![token_balance(MINT, TRADER, "0", 6)],
        ...
        let event = decode(&result, &instruction(&DISC_BUY_EXACT_OUT), &whales)
            .expect("buy_exact_out decodes");
    ...
    fn returns_none_when_program_id_mismatches() {
        ...
        let mut ix = instruction(&DISC_BUY_EXACT_IN);
