...
//!
//! Source: `docs/dex-reference.md` §2 (official pump-public-docs IDL
//! `github.com/pump-fun/pump-public-docs/idl/pump.json`) and §0 (cross-cutting
...
/// PumpFun bonding-curve program. Source: docs/dex-reference.md §2.
pub const PROGRAM_ID: &str = "6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P";

// Discriminators = sha256("global:<name>")[..8]; source docs/dex-reference.md §2.
...
pub fn decode(
    ...
    // 1. Defensive: confirm this instruction targets PumpFun.
    if instruction.program_id() != Some(PROGRAM_ID) {
        return None;
    }

    // 2. Discriminator → action (buy/sell). Anything else (create, etc.) is skipped.
...
mod tests {
    ...
    fn instruction(disc: &[u8; 8]) -> UiInstruction {
        let some_args = [0u8, 0, 0, 0, 0, 0, 0, 0, 1]; // amount + max_sol_cost-ish; ignored
    ...
    fn returns_none_for_non_buy_sell_discriminator() {
        // A create/other discriminator (not buy/sell) must be skipped.
